	...
	$.ajax({
		...
		$("#name")
			...
			.on("typeahead:selected", function (event, data) {
				$(event.currentTarget)
					.parent()
					.next("input[name=" + event.currentTarget.name + "-key]")
					.val(data.ID);
			});
	});
}
...
function viewAddCategory() {
	...
	$("#foot input").remove();
	$("#foot").append(/*html*/`<br><input class="ui green button" type="submit">`);
...
function addicon() {
	...
	$("#view").append(/*html*/`
		<div class="icon[${len}]" id="icon[${len} ]">
		<br />
		<img width="304" class="img-thumbnail preview[${len}]" />
		<input
			class="form-control"
			type="file"
			onchange="preview()"
...
function viewAddProperty() {
	...
	$("#view").append(/*html*/ `
		...
		<div class="field">
			...
			<input
				class="form-control typeahead rtl"
				type="text"
				placeholder="برجاء الادخال بالعربي"
				name="nameAr"
			/>
			<input type="hidden" name="nameAr-key" />
		...
		<div class="field">
			<div class="ui checkbox">
				<label class="checkbox-wrap"> Filter by it </label>
				<input
					class="thecheckbox"
					name="filterable"
					type="checkbox"
	...
	$("[name=property]").append(/*html*/`<option value="0">No property</option>`);
	$("#value").append(/*html*/`<option value="0">No value</option>`);
...
function viewSubProperty() {
	...
	$("#view").append(/*html*/ `
		...
		<select class="form-control" name="property[${len}]"></select>
		<br />
		<label>Property value:</label>
		<select class="form-control" name="value[${len}]"></select>
		<div style="width: 100%; margin-top: 10px">
			<a
				href="#"
				class="ui red button"
				style="float: right"
				onclick="removeme('#property${len}')"
				>X</a
			>
		</div>
	</div>
		`);
	$($("#property" + len + ' [name^="property"]')[0]).append(
		/*html*/`<option value="0">No property</option>`
	);
	$($("#property" + len + ' [name^="value"]')[0]).append(
		/*html*/`<option value="0">No value</option>`
	);

	$($("#property" + len + ' [name^="property"]')[0]).on(
		"change",
		{ name: len },
		ajaxValue
	);
	...
	ajaxProperty(
		$($("#property" + len + ' [name^="property"]')[0]),
		$($("#property" + len + ' [name^="value"]')[0]),
		false
	);
}
...
function viewAddSubCategory() {
	...
	$("#view").append(/*html*/ `
		...
		<div class="field">
			<div class="ui checkbox">
				<label class=""> Show up in the quick details </label>
				<input class="quickdetails" name="quickdetails" type="checkbox" />
			</div>
			<div class="ui checkbox">
				<label class="" style="float: right"> Feature as a hot item </label>
				<input class="quickdetails" name="quickdetails" type="checkbox" />
			</div>
		</div>
		`);
	$("#foot").prepend(
    /*html*/ `<a href="#" id="plusprop">Add property (+)</a><br><br>`
	);
	$("[name=view]").prop("action", "class_cms.php?method=setSubCategory");
	ajaxCategory();
	$("#foot").append(
    /*html*/ `<br><input class="ui green button" type="submit">`
	);
	$(".checkbox").checkbox();
	$(".dropdown").dropdown();
	cardPreview();
	$(window).on("click keyup change", function () {
		cardPreview();
	});
}
...
function viewAddLocale() {
	...
	$("#view").append(/*html*/ `
		...
		<div class="field">
			<input
				class="form-control"
				placeholder="ValueInLowercaseWithNoSpaces"
				name="key"
			/>
		</div>
		<div class="field">
...
function preview() {
	...
	for (var i = 0; i < icons.length; i++) {
		uploadimg(
			$(icons[i]).find("img").first()[0],
			$(icons[i]).find("input").first()[0]
		);
	}
}
