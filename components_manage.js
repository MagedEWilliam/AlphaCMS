...
function ajaxPages() {
	$.ajax({
		...
		for (var i = 0; i < data.length; i++) {
			...
			let checked = false;
			if (data[i].Available == '1' && Number(data[i].hascontent) > 0.00) {
				checked = true;
			}
			...
			<li class="ui-state-default">
				...
				<div class="ui checkbox floatright">
					...
					<input type="checkbox" name="visibility[${i}]" ${checked && 'checked'}>
				</div>
			</li>
			`;

			$('#sortable').append(vie + /*html*/`</div></li>`);
		}
		$("#sortable").sortable();
		$('.checkbox').checkbox();
	});
...
function viewNewPart() {
	var view = /*html*/`
		...
		<div class="ui bottom attached tab segment active" data-tab="en">
			...
			<div class="ui tab active" data-tab="editor">
				<textarea id="summernote" name="content"></textarea>
			</div>
			<div class="ui tab" data-tab="code">
				<textarea name="HTMLCODE" placeholder="HTML CODE"></textarea>
			</div>
		</div>
		...
		<div class="ui bottom attached tab segment" data-tab="ar">
			...
			<div class="ui tab active" data-tab="editorAr">
				<textarea id="summernoteAr" name="contentAr"></textarea>
			</div>
			<div class="ui tab" data-tab="codeAr">
				<textarea name="HTMLCODEAr" placeholder="HTML CODE"></textarea>
			</div>
		</div>
		...
		<div class="ui bottom attached tab segment" data-tab="ch">
			...
			<div class="ui tab active" data-tab="editorCh">
				<textarea id="summernoteCh" name="contentCh"></textarea>
			</div>
			<div class="ui tab" data-tab="codeCh">
				<textarea name="HTMLCODECh" placeholder="HTML CODE"></textarea>
			</div>
		</div>
	`;
...
let heyLookImGlopal_State_content = [];
let heyLookImGlopal_State_contentAr = [];
let heyLookImGlopal_State_contentCh = [];
...
function getParts() {
	...
	var view = /*html*/`
		...
		<div class="ui bottom attached tab segment active" data-tab="en">
			...
			<div class="ui tab active" data-tab="code">
				<textarea name="HTMLCODE" placeholder="HTML CODE"></textarea>
			</div>
		</div>
		...
		<div class="ui bottom attached tab segment" data-tab="ar">
			...
			<div class="ui tab" data-tab="editorAr">
				<textarea id="summernoteAr" name="contentAr"></textarea>
			</div>
			<div class="ui tab active" data-tab="codeAr">
				<textarea name="HTMLCODEAr" placeholder="HTML CODE"></textarea>
			</div>
		</div>
		...
		<div class="ui bottom attached tab segment" data-tab="ch">
			...
			<div class="ui tab" data-tab="editorCh">
				<textarea id="summernoteCh" name="contentCh"></textarea>
			</div>
			<div class="ui tab active" data-tab="codeCh">
				<textarea name="HTMLCODECh" placeholder="HTML CODE"></textarea>
			</div>
		</div>
	...
	ajaxPart();
	widdenview('sixteen', 'three');
	$('#rightview').append(/*html*/`<div class="field"><input class="ui green button" type="submit" value="Save"></div>`);
...
function ajaxPart() {
	$('[name=part]').html('');
	$.getJSON("class_cms.php?method=getPart&page=" + $('[name=page]').val(), (data) => {
		...
		const id = data[i].ID;
		for (var i = 0; i < data.length; i++) {
			$('[name=part]').append(/*html*/`<option value="${id}">${data[i].partid}</option>`);
			heyLookImGlopal_State_content[id] = safeContent(data[i].content);
			heyLookImGlopal_State_contentAr[id] = safeContent(data[i].contentAr);
			heyLookImGlopal_State_contentCh[id] = safeContent(data[i].contentCh);
		}
	});
}

function ajaxCards() {
	$.getJSON("class_cms.php?method=getSubCategory", (data) => {
		for (var i = 0; i < data.length; i++) {
			var vie = /*html*/`
...
function _ajaxPart() {
	$.getJSON("class_cms.php?method=getPagePart&page=" + $('[name=page]').val(), (data) => {
		$('#sortable').html('');
		for (var i = 0; i < data.length; i++) {
			var vie = /*html*/`<li class="ui-state-default"><i class="ui icon resize horizontal"><input type="hidden" name="pagenum[${i}]" value="${data[i].ID}"></i><b>${data[i].partid}</b></li>`;
			$('#_' + data[i].ID).remove();
			$('#sortable').append(vie);
		}
	});
...
function ajaxPart_drag() {
	$.getJSON("class_cms.php?method=getPart&page=all", (data) => {
		$('#sortable2').html('');
		for (var i = 0; i < data.length; i++) {
			var vie = /*html*/`<li class="ui-state-default" id="_${data[i].ID}"><i class="ui icon resize horizontal"><input type="hidden" name="pagenum[${i}]" value="${data[i].ID}"></i><b>${data[i].partid}</b></li>`;
