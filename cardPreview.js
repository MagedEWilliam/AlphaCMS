function card(target, ItemProp, classes){
	...
	<a class="ui yellow small button" id="'+ItemProp.item.code+'" href="#"><p class="goodtimes">Details</p></a>\
	<div class="or"></div>\
	<a class="ui '+classes+' small button"  id="cart_'+ItemProp.item.code+'" ><p class="goodtimes">To Cart</p></a>\
	</div>\
	</div>\
	</div>\
	</div>\
	...
	function trtd(prop){
		...
		$.each($('.properties'), function(i, value) {
			...
			var proper = $(value).find('select')[0];
			var val = $(value).find('select')[1];
			if( $(proper).find('option:selected').text() != "Subcategory"){
				temp += '<tr>'
				temp += rtlSlpadrPad('rtl Fixedtd slpad rpad', $(proper).find('option:selected').text() );
				temp += '<td class="slpad">';
				temp += lblSlpadrPad( $(val).find('option:selected').text() );
